...
const createProduct = async (product, variants, images) => {
  ...
  try {
    ...
    variants = variants.map((variant) => [result.insertId, ...variant]);
    await conn.query('INSERT INTO variant(product_id, color_id, size, stock) VALUES ?', [variants]);
    images = images.map((image) => [result.insertId, ...image]);
...
const getHotProducts = async (hotId) => {
  const productQuery =
    'SELECT product.* FROM product INNER JOIN hot_product ON product.id = hot_product.product_id WHERE hot_product.hot_id = ? ORDER BY product.id';
...
const getProductsVariants = async (productIds) => {
  const queryStr =
    'SELECT * FROM variant INNER JOIN color ON variant.color_id = color.id WHERE product_id IN (?)';
